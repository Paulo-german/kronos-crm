...
import { findDealWithRBAC } from '@/_lib/rbac'
import {
  serializeFieldValue,
  parseFieldOptions,
} from '@/_lib/custom-fields/serialize'
import { updateDealCustomFieldsSchema } from './schema'

export const updateDealCustomFields = orgActionClient
  .schema(updateDealCustomFieldsSchema)
  .action(async ({ parsedInput: data, ctx }) => {
    // 1. RBAC + ownership do deal (MEMBER só edita os próprios; valida org)
    ...
    await db.$transaction([
      ...deletions.map((fieldDefinitionId) =>
        db.customFieldValue.deleteMany({
          where: { fieldDefinitionId, entityId: data.dealId },
        }),
      ),
      ...upserts.map((upsert) =>
    ...
    revalidateTag(`deal-custom-fields:${data.dealId}`)
