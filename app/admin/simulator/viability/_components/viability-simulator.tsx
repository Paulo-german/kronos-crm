...
function computeViabilityMetrics(
  ...
  const totalCustomers = baseline.reduce(
  ...
  if (totalCustomers > 0 && targetMargin > 0) {
    ...
    if (contribPerCustomer > 0) {
      const gap =
        totalFixed - (totalRevenue * (1 - targetMargin / 100) - totalAiCost)
...
export function ViabilitySimulator({ baseline }: ViabilitySimulatorProps) {
  ...
  const results = useMemo(
    () =>
      computeViabilityMetrics(baseline, {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <div className="border-t pt-3 text-sm text-muted-foreground md:col-span-2">
              Total fixo:{' '}
        ...
        <Card>
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-xs">
              O custo/crédito é exibido como &quot;Custo/crédito (API)&quot; no
              Simulador de Créditos IA.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2 md:col-span-2">
              ...
              <NumericFormat
                ...
                onValueChange={(values) =>
                  setCostPerCreditUsd(values.floatValue ?? 0)
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <NumericFormat
                ...
                value={rate}
                onValueChange={(values) =>
                  setRate(values.floatValue ?? DEFAULT_RATE)
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <NumericFormat
                ...
                onValueChange={(values) =>
                  setTargetMarginPct(values.floatValue ?? 0)
                }
              />
            </div>
          </CardContent>
          ...
          <Table>
            ...
            <TableBody>
              ...
              <TableRow className="border-t-2 font-semibold">
                ...
                <TableCell />
                <TableCell className="text-right">
                  {fmt.brl(results.totalRevenue)}
                </TableCell>
                <TableCell className="text-right">
                  {fmt.brl(results.totalAiCost)}
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
        ...
        <Card className="border-dashed">
          <CardContent className="flex items-center gap-3 py-8">
            ...
            <p className="text-sm text-muted-foreground">
              Nenhuma assinatura ativa encontrada. Os resultados aparecerão
              assim que houver clientes com assinatura ativa ou em trial.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent className="space-y-1 text-sm">
              ...
              <div className="pt-1">
                <MarginBadge
                  marginPct={results.marginPct}
                  targetPct={results.targetMargin}
                />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardDescription>
                Break-even — clientes adicionais
              </CardDescription>
            ...
            <CardContent className="text-sm text-muted-foreground">
              {results.extraCustomersNeeded === null ? (
                <p>
                  Com os parâmetros atuais, cada novo cliente gera prejuízo no
                  alvo de margem. Revise os custos ou reduza o alvo.
                </p>
              ) : results.extraCustomersNeeded <= 0 ? (
                <p>
                  A margem atual ({fmt.pct(results.marginPct)}) já supera o alvo
                  de {results.targetMargin}%. Nenhum cliente adicional
                  necessário.
                </p>
              ) : (
                <p>
                  Clientes no mix atual para atingir {results.targetMargin}% de
                  margem. Base atual: {fmt.number(results.totalCustomers)}.
                </p>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-1 text-sm">
              ...
              results.priceAdjustmentPct > 0 ? (
                ...
                <p className="text-muted-foreground">
                  Os preços atuais já são suficientes para atingir a margem
                  alvo.
                </p>
              ) : (
                <p className="text-muted-foreground">
