...
interface DealFilterBadgesProps {
  filters: DealFilters
...
export function DealFilterBadges({
  filters,
  onFiltersChange,
  onClearFilters,
  hasActiveFilters,
}: DealFilterBadgesProps) {
      ...
      {filters.status.map((status) => (
        ...
        >
          ...
          <button
            ...
            aria-label={`Remover filtro de status ${getStatusLabel(status)}`}
          >
            <X className="h-3 w-3" />
          </button>
        </Badge>
      ...
      {filters.priority.map((priority) => (
        ...
        >
          ...
          <button
            ...
            className="ml-1 rounded-full p-0.5 hover:bg-muted-foreground/20"
            aria-label={`Remover filtro de prioridade ${getPriorityLabel(priority)}`}
          >
            <X className="h-3 w-3" />
          </button>
        </Badge>
