    ...
    useMemo(() => {
      return {
        ...
        upcomingAppointments: appointments.filter(
          (appointment) =>
            isFuture(startOfDay(appointment.startDate)) &&
            !isToday(appointment.startDate),
        ),
        pastAppointments: appointments
          .filter(
            (appointment) =>
              isPast(startOfDay(appointment.startDate)) &&
              !isToday(appointment.startDate),
          )
          .slice(-10)
          .reverse(),
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">
              {professional.professionalServices.length}
            </p>
            <p className="text-xs text-muted-foreground">
              serviço(s) disponível(is)
            </p>
          </CardContent>
        </Card>
      </div>
...
function AppointmentRow({ appointment, showDate, muted }: AppointmentRowProps) {
  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-md border p-3 ${muted ? 'opacity-60' : ''}`}
    >
