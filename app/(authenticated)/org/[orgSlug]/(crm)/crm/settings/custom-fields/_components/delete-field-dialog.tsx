...
export const DeleteFieldDialog = ({
  ...
  const recordLabels = ENTITY_RECORD_LABELS[definition?.entityType ?? 'CONTACT']
    ...
    <div className="space-y-3 text-center">
      ...
      {hasValues ? (
        <div className="flex items-start gap-3 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-3 text-left text-sm text-destructive">
          ...
          <div className="space-y-1">
            <p className="font-semibold">
              {valueCount}{' '}
              {valueCount === 1
                ? `${recordLabels.singular} possui`
                : `${recordLabels.plural} possuem`}{' '}
              dados neste campo
            </p>
            <p className="text-destructive/80">
              Ao confirmar, todos esses dados serão{' '}
              <span className="font-semibold">permanentemente apagados</span> e
        ...
        <p className="text-xs text-muted-foreground">
          {recordLabels.none} possui dados preenchidos neste campo.
        </p>
      )}
    </div>
  )
