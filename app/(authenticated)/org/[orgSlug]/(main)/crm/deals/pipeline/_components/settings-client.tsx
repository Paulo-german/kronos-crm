...
export function SettingsClient({ pipeline }: SettingsClientProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Dialog de exclusão simples (etapa sem deals) — fora do Sheet para evitar Dialog aninhado */}
      ...
      {/* Dialog de exclusão com migração (etapa com deals) — fora do Sheet para evitar Dialog aninhado */}
      {deleteTarget && deleteTarget.dealCount > 0 && (
        <DeleteStageDialog
          open={!!deleteTarget}
          onOpenChange={(open) => !open && setDeleteTarget(null)}
