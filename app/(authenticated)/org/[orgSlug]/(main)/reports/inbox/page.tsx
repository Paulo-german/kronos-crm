...
export default async function InboxReportPage({ params, searchParams }: InboxReportPageProps) {
  ...
  // Busca etiquetas para o filtro local
  const conversationLabels = await getConversationLabels(ctx.orgId)

  // Filtros de Inbox — assignee vem do shell global (já parseado)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Filtros locais (canal, etiqueta, status, ia/humano) */}
      ...
      {/* Row 5: Performance por Atendente (elevated only) */}
      {elevated && (
        <Suspense fallback={<InboxChartSkeleton />}>
