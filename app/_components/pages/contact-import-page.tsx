...
import { ImportClient } from '@/_components/contacts/import/import-client'
import Header, {
  HeaderLeft,
  HeaderTitle,
  HeaderSubTitle,
...
interface ImportPageProps {
  ...
  // Produto que renderiza o import (crm | prospection | inbox | agents)
  basePath?: string
}

const ImportPage = async ({ params, basePath = 'crm' }: ImportPageProps) => {
  ...
  return (
    <div className="space-y-6">
      <Button variant="ghost" size="sm" className="gap-2" asChild>
        <Link href={`/org/${orgSlug}/${basePath}/contacts`}>
      ...
      {!quota.withinQuota ? (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center gap-4">
            <span>
              Você atingiu o limite de {quota.limit} contatos do seu plano. Faça
              upgrade para importar mais contatos.
            </span>
            <Button variant="outline" size="sm" asChild>
              <Link href={`/org/${orgSlug}/settings/billing`}>Ver planos</Link>
            </Button>
          </AlertDescription>
        </Alert>
      ) : (
