...
const ContactDetailPage = async ({ params, contactsHref }: ContactDetailPageProps) => {
  const { id, orgSlug } = await params
  const ctx = await getOrgContext(orgSlug)

  const contact = await getContactById(id, ctx)
  if (!contact) {
    redirect(contactsHref ? contactsHref(orgSlug) : `/org/${orgSlug}/crm/contacts`)
  }

  const [companies, members, lifecycleHistory, allFieldDefinitions, customFieldValues, privacy] =
    await Promise.all([
