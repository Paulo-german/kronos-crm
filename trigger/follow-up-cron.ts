...
async function revalidateConversationCache(
  ...
  if (!appUrl || !secret) {
    logger.warn(
      '[follow-up-cron] Missing NEXT_PUBLIC_APP_URL or INTERNAL_API_SECRET, skipping cache revalidation',
    )
    return
  }
...
export const followUpCron = schedules.task({
  ...
  run: async () => {
        ...
        OR: [
          {
            inbox: {
              agentId: { not: null },
              agent: { isActive: true, NOT: { agentVersion: 'single-v2' } },
            },
          },
          {
            inbox: {
              agentGroupId: { not: null },
              agentGroup: { isActive: true },
            },
          },
        ],
      },
    ...
    logger.info('[follow-up-cron] Processing batch', {
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            logger.warn('[follow-up-cron] Invalid FUP business hours config', {
              convId: conv.id,
            })
          } else {
            const bhConfig =
              resolvedAgent.followUpBusinessHoursConfig as BusinessHoursConfig
            ...
            if (!isOpen) {
              ...
              const nextOpeningTime = getNextOpeningTime(timezone, bhConfig)
        ...
        if (conv.inbox.connectionType === 'SIMULATOR') {
          ...
          if (nextFollowUpSim) {
            const nextFollowUpAtSim = new Date(
            ...
            await db.conversation.update({
              ...
              data: {
                ...
                nextFollowUpAt: nextFollowUpAtSim,
              },
            })
          } else {
            await db.conversation.update({
    ...
    logger.info('[follow-up-cron] Done', {
      processed,
      skipped,
      errors,
